...
trait UserQueries {
  ...
  object Query {
    ...
    def unfriend(owner: String, friend: String) = execute("DELETE FROM friends USING users AS u1, users AS u2 WHERE owner=u1.id AND u1.nickname=? AND friend=u2.id AND u2.nickname=?", owner, friend)
